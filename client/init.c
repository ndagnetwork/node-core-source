...
int g_xdag_run = 0;
int g_is_exwallet = 0; //new add
int g_balance_notify = 0; //new add
...
int xdag_init(int argc, char **argv, enum ndag_node node)
{
	...
	if (node==NDAG_NODE_CORE) {
		printf("%s node-core, version %s.\n", g_progname, XDAG_VERSION);
	}
	else if (node == NDAG_NODE_EXWALLET) {
		g_is_exwallet = 1;
		printf("%s ex-wallet, version %s.\n", g_progname, XDAG_VERSION);
	}
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (ARG_EQUAL(argv[i], "-a", "")) { /* miner address */
			...
			g_disable_mining = 1;
		} else if (ARG_EQUAL(argv[i], "", "-balance-notify")) { /* enable balance notify */
			g_balance_notify = 1;
		} else {
			printUsage(argv[0]);
			return 0;
	...
	if(!is_pool && pool_arg == NULL) {
		if(!xdag_search_pool(g_pool_address)) {
			return -1;
		}
		is_miner = 1;
	...
	if (is_miner && (is_pool || bindto || n_addrports)) {
		printf("Miner can't be a pool or have directly connected to the NDAG network.\n");
		return -1;
	}
