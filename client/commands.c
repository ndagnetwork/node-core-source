...
void processGetNewAddressCommand(char *nextParam, FILE *out);
...
int xdag_com_getnewaddress(char *, FILE*);
...
XDAG_COMMAND commands[] = {
	{ "getnewaddress",1,xdag_com_getnewaddress},
...
int xdag_com_getnewaddress(char* args, FILE* out)
{
	processGetNewAddressCommand(args, out);
	return 0;
}
...
void processGetNewAddressCommand(char *nextParam, FILE *out)
{
	...
	if (res < 0) {
		fprintf(out, "Can't generate new address.\n");
	}
	else {
		char address[SIZE_MAX_ADDRESS];
		ndag_hash2address(hash, address);
		fprintf(out, "Address %s generated.\n", address);
	}
}
...
void processStatsCommand(FILE *out)
{
	if(g_is_miner) {
		...
		fprintf(out, "Statistics for ours and maximum known parameters:\n"
			...
			amount2xdags(ndag_get_supply(g_xdag_stats.nmain)),
			amount2xdags(ndag_get_supply(g_xdag_stats.total_nmain)),
...
long double hashrate(xdag_diff_t *diff)
{
	...
	//shown pool and network hashrate seems to be around 35% higher than real, to consider *(0.65) about correction. Deeper study is needed.
	//return ldexpl(expl(sum), -58); 
	double a = expl(sum);
	return ldexpl(a, -36)*(0.45);
}
...
int xdag_do_xfer(void *outv, const char *amount, const char *address, const char *fromaddress, int isGui)
{
	...
	//add new
	if (fromaddress) {
		if (xdag_address2hash(fromaddress, xfer.fromHash)) {
			if (out) {
				fprintf(out, "Xfer: incorrect source address.\n");
			}
			return 1;
		}
		if (!strcmp(fromaddress, address)) {
			if (out) {
				fprintf(out, "Xfer: source and destination are the same.\n");
			}
			return 1;
		}
	}
	...
	if(xdag_address2hash(address, xfer.fields[XFER_MAX_IN].hash)) {
		if(out) {
			fprintf(out, "Xfer: incorrect destination address.\n");
		}
		return 1;
	}
...
{
	char address_from[SIZE_MAX_ADDRESS], address_to[SIZE_MAX_ADDRESS];
...
{
	char address_l[SIZE_MAX_ADDRESS], address_r[SIZE_MAX_ADDRESS];
...
int xdag_show_state(xdag_hash_t hash)
{
	...
	if(g_xdag_state < XDAG_STATE_XFER) {
		...
		sprintf(balance, "%.9Lf", amount2xdags(xdag_get_balance(hash)));
	}
	if(!hash) {
...
{
	return g_xdag_extstats.hashrate_s / (1024);
}

int read_command(char *cmd)
{
#if !defined(_WIN32) && !defined(_WIN64)
	//char* line = linenoise("ndag> ");
	char* line = linenoise(g_is_exwallet?"exwallet> ":"ndag> ");
