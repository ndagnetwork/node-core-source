...
#define DEV_ADDRESS                       "EzRsgxPcXRgkFJ8YLnPSN6p7Rv5F83hjs"   /* dev fee wallet address */
#define SHARES_PER_TASK_LIMIT              20                                   /* maximum count of shares per task */
#define DEFAUL_CONNECTIONS_PER_MINER_LIMIT 100
#define MINERID_HEADER_WORD             0x4d6b9853u
...
struct miner_pool_data {
	struct xdag_field miner_address;
...
//new add
typedef struct addresses_list_element {
	xdag_hash_t hash;
	time_t balance_refreshed_time;
	struct addresses_list_element *next;
} addresses_list_element;
//***
...
struct connection_pool_data {
	...
	addresses_list_element *miner_addresses_list_head;//new add
	uint32_t ip;
	uint16_t port;
...
static double g_pool_fee = 0, g_pool_reward = 0, g_pool_direct = 0, g_dev_fee = 10;
static struct xdag_block *g_firstblock = 0, *g_lastb = 0;

static int g_stop_general_mining = 1;

static struct miner_pool_data g_pool_miner;
static struct miner_pool_data g_dev_miner;
...
int xdag_initialize_pool(const char *pool_arg)
{
	...
	memset(&g_dev_miner, 0, sizeof(struct miner_pool_data));

	xdag_get_our_block(g_pool_miner.miner_address.data);
...
void *general_mining_thread(void *arg)
{
	...
	while(!g_stop_general_mining) {
		xdag_create_block(0, 0, 0, 0, xdag_main_time() << 16 | 0xffff, 0);
	}
...
int xdag_pool_set_config(const char *pool_config)
{
	...
	if(pool_config) {
		...
		if(g_dev_fee + g_pool_fee > 1)
			g_pool_fee = 1 - g_dev_fee;
	}
	...
	if(pool_config) {
		...
		if(g_pool_reward < 0)
			g_pool_reward = 0;
		if(g_dev_fee + g_pool_fee + g_pool_reward  > 1)
			g_pool_reward = 1 - g_dev_fee - g_pool_fee ;
	}
	...
	if(pool_config) {
		...
		if(g_pool_direct < 0)
			g_pool_direct = 0;
		if(g_dev_fee + g_pool_fee + g_pool_reward + g_pool_direct > 1)
			g_pool_direct = 1 - g_dev_fee - g_pool_fee - g_pool_reward;
	}
...
char *xdag_pool_get_config(char *buf)
{
	...
	sprintf(buf, "%d:%d:%d:%.2lf:%.2lf:%.2lf:%.2lf", g_max_connections_count, g_max_miner_ip_count, g_connections_per_miner_limit,
		g_pool_fee * 100, g_pool_reward * 100, g_pool_direct * 100, g_dev_fee * 100);
...
static void close_connection(connection_list_element *connection, const char *message)
{
	...
	//new add
	addresses_list_element *el, *tmp;
	LL_FOREACH_SAFE(conn_data->miner_addresses_list_head, el, tmp)
	{
		LL_DELETE(conn_data->miner_addresses_list_head, el);
		free(el);
	}
	//***
	...
	if(conn_data->miner) {
		char address_buf[SIZE_MAX_ADDRESS];
		xdag_hash2address(conn_data->miner->miner_address.data, address_buf);
...
static void calculate_nopaid_shares(struct connection_pool_data *conn_data, struct xdag_pool_task *task, xdag_hash_t hash)
{
	...
	if(conn_data->task_time <= task_time) { // At the beginning conn_data->task_time=0. conn_data->task_time > task_time isn't accepted.
		...
		// CONFIRMATION_COUNT-1=15d=1111b, thus it just cut task_time to its 4 least significant bit
		...
		// and 2^64 is 0xFFFFFFFFFFFFFFFF, ldexp(diff, -64) will return exactly 1 if
...
static int register_new_miner(connection_list_element *connection)
{
	...
	{
		if(memcmp(elt->miner_data.miner_address.data, conn_data->data, sizeof(xdag_hashlow_t)) == 0) {
	...
	if(!exists) {
		...
		memcpy(new_miner->miner_data.miner_address.data, conn_data->data, sizeof(struct xdag_field));
	...
	if(!conn_data->block_size && conn_data->data[0] == BLOCK_HEADER_WORD) {
		...
		if(conn_data->block_size == XDAG_BLOCK_FIELDS) {
			...
			if(crc == crc_of_array((uint8_t*)conn_data->block, sizeof(struct xdag_block))) {
				...
				//new add
				struct xdag_block newBlock;
				xdag_hash_t hash;
				addresses_list_element *new_address = malloc(sizeof(addresses_list_element));
				if (!new_address) return -1;
				memset(new_address, 0, sizeof(addresses_list_element));
				memcpy(&newBlock, conn_data->block, sizeof(struct xdag_block));
				newBlock.field[0].transport_header = 0;
				xdag_hash(&newBlock, sizeof(struct xdag_block), new_address->hash);

				LL_APPEND(conn_data->miner_addresses_list_head, new_address);
				//***

				if(!g_firstblock) {
					g_firstblock = g_lastb = conn_data->block;
				} else {
...
static int is_worker_name_received(connection_list_element *connection)
{
	...
	if(conn_data->nfield_in == 17 && conn_data->data[0] == MINERID_HEADER_WORD) {
...
static int process_received_share(connection_list_element *connection)
{
	...
	if(conn_data->state == UNKNOWN_ADDRESS) {
		...
		if(memcmp(conn_data->miner->miner_address.data, conn_data->data, sizeof(xdag_hashlow_t)) != 0) {
			close_connection(connection, "Wallet address was unexpectedly changed");
			return 0;
		}
		memcpy(conn_data->miner->miner_address.data, conn_data->data, sizeof(struct xdag_field));	//TODO:do I need to copy whole field?
	}
	...
	if(share_can_be_accepted(conn_data->miner, (uint64_t*)conn_data->data, task_index)) {
		...
		xdag_hash_final(task->ctx0, conn_data->data, sizeof(struct xdag_field), current_hash);
		xdag_set_min_share(task, conn_data->miner->miner_address.data, current_hash);
		update_mean_log_diff(conn_data, task, current_hash);
		calculate_nopaid_shares(conn_data, task, current_hash);
	}
...
static int send_data_to_connection(connection_list_element *connection, int *processed)
{
	...
	struct xdag_field data[XDAG_BLOCK_FIELDS];
	//***
	int fields_count = 0;
	struct connection_pool_data *conn_data = &connection->connection_data;
	...
	if(conn_data->task_index < task_index) {
		...
		fields_count = 2;
		memcpy(data, task->xdagfield, fields_count * sizeof(struct xdag_field));
	} 
	//new add
	/*else if(conn_data->miner && t - conn_data->balance_refreshed_time >= 10) {  //refresh balance each 10 seconds
		//TODO: optimize refreshing of balance
		conn_data->balance_refreshed_time = t;
		memcpy(data[0].data, conn_data->miner->id.data, sizeof(xdag_hash_t));
		data[0].amount = xdag_get_balance(data[0].data);
		fields_count = 1;
	}*/
	else {
		addresses_list_element *el;
		LL_FOREACH(conn_data->miner_addresses_list_head, el) {
			if (t - el->balance_refreshed_time >= 10 && fields_count < XDAG_BLOCK_FIELDS) {
				el->balance_refreshed_time = t;
				memcpy(data[fields_count].data, el->hash, sizeof(xdag_hash_t));
				data[fields_count].amount = xdag_get_balance(data[fields_count].data);
				fields_count++;
			}
		}
	}
...
void *pool_block_thread(void *arg)
{
	...
	for(;;) {
		...
		if(g_firstblock) {
			b = g_firstblock;
			g_firstblock = (struct xdag_block *)(uintptr_t)b->field[0].transport_header;
			if(!g_firstblock) g_lastb = 0;
		} else {
			b = 0;
		}
...
#define diff2pay(d, n) ((n) ? exp((d) / (n) - 6) * (n) : 0)
...
static inline double miner_calculate_unpaid_shares(struct miner_pool_data *miner)
{
	...
	double shares = (count ? exp(sum / count-6) * count : 0); // (count ? exp(sum / count - 20) * count : 0);
	return shares;
}
...
static int precalculate_payments(uint64_t *hash, int confirmation_index, struct payment_data *data, double *diff, double *prev_diff, uint64_t *nonce)
{
	...
	if(g_dev_fee) {
		if(g_dev_miner.state == MINER_UNKNOWN) {
			xdag_time_t t;
			if(!xdag_address2hash(DEV_ADDRESS, g_dev_miner.miner_address.hash) && xdag_get_block_pos(g_dev_miner.miner_address.hash, &t) >= 0) {
				g_dev_miner.state = MINER_SERVICE;
			}
		}

		if(g_dev_miner.state != MINER_UNKNOWN) {
			data->dev = data->balance * g_dev_fee;
			data->pay -= data->dev;
		}
	}
	...
	LL_FOREACH(g_miner_list_head, elt)
	{
		...
		if(data->reward_index < 0 && !memcmp(nonce, miner->miner_address.data, sizeof(xdag_hashlow_t))) {
...
static void transfer_payment(struct miner_pool_data *miner, xdag_amount_t payment_sum, struct xdag_field *fields, int payments_per_block, int *field_index)
{
	...
	memcpy(fields[*field_index].data, miner->miner_address.data, sizeof(xdag_hashlow_t));
...
static void do_payments(uint64_t *hash, int payments_per_block, struct payment_data *data, double *diff, double *prev_diff)
{
	...
	if(g_dev_miner.state != MINER_UNKNOWN) {
		transfer_payment(&g_dev_miner, data->dev, fields, payments_per_block, &field_index);
	}
...
int pay_miners(xdag_time_t time)
{
	...
	int keyidx = xdag_get_key_idx(hash);
	if(keyidx < 0) return -4;

	if(!xdag_wallet_default_key(&defkeyidx)) return -5;

	const int payments_per_block = (keyidx == defkeyidx ? 12 : 10);
...
void remove_inactive_miners(void)
{
	...
	LL_FOREACH_SAFE(g_miner_list_head, elt, eltmp)
	{
		if(elt->miner_data.state == MINER_ARCHIVE && miner_calculate_unpaid_shares(&elt->miner_data) == 0.0) {
			xdag_hash2address(elt->miner_data.miner_address.data, address);
...
static int print_miner(FILE *out, int index, struct miner_pool_data *miner, int print_connections)
{
	...
	char address_buf[SIZE_MAX_ADDRESS];
	xdag_hash2address(miner->miner_address.data, address_buf);
...
static void print_connection(FILE *out, int index, struct connection_pool_data *conn_data)
{
	...
	if(conn_data->miner) {
		xdag_hash2address(conn_data->miner->miner_address.data, address);
	} else {
		strcpy(address, "-                                   ");
	}
...
int xdag_print_miners(FILE *out, int printOnlyConnections)
{
	fprintf(out, "List of miners:\n"
		" NN  Address for payment to              Status   IP/port                in/out            unpaid shares  worker name   MH/s\n"
		"-----------------------------------------------------------------------------------------------------------------------------------\n");
	...
	if(conn_data->task_time < task_time) {
		if(conn_data->task_time != 0) {
			conn_data->mean_log_difficulty =
				moving_average(
					conn_data->mean_log_difficulty, 
					diff2log(xdag_hash_difficulty(conn_data->last_min_hash)), 
	...
	if(conn_data->miner->task_time < task_time) {
		if(conn_data->miner->task_time != 0) {
			conn_data->miner->mean_log_difficulty =
				moving_average(
					conn_data->miner->mean_log_difficulty, 
					diff2log(xdag_hash_difficulty(conn_data->miner->last_min_hash)), 
...
long double diff2log(xdag_diff_t diff)
{
	...
	// diff = diff / 0xffffffffffffffff = diff >> 64
...
static void miner_print_time_intervals(struct miner_pool_data *miner, int current_interval_index, xdag_time_t current_task_time, FILE *out)
{
	...
	for(int i = 0; i < CONFIRMATIONS_COUNT; ++i) {
		...
		int is_reward = memcmp(g_xdag_mined_nonce[i], miner->miner_address.data, sizeof(xdag_hashlow_t)) == 0;
...
static void print_miner_stats(struct miner_pool_data *miner, FILE *out)
{
	...
	uint64_t *h = miner->miner_address.data;
...
int xdag_print_miner_stats(const char* address, FILE *out)
{
	...
	LL_FOREACH(g_miner_list_head, elt)
	{
		if(memcmp(elt->miner_data.miner_address.data, hash, sizeof(xdag_hashlow_t)) == 0) {
			struct miner_pool_data *miner = &elt->miner_data;
			exists = 1;
