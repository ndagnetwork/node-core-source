...
cJSON * method_ndag_##command (struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version); \
cJSON * method_ndag_##command (struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version) { \
...
/* method: xdag_version */
cJSON * method_version(struct xdag_rpc_context *ctx, cJSON *params, cJSON *id, char *version);
...
cJSON * method_accounts(struct xdag_rpc_context *ctx, cJSON *params, cJSON *id, char *version);

/* method: xdag_new_account */
cJSON * method_getnewaddress(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version);

/* method: xdag_get_balance */
cJSON * method_getbalance(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version);

/* method: xdag_do_xfer */
cJSON * method_send(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version);
...
cJSON * method_listtransactions(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version);
...
/* method: xdag_get_new_address */
cJSON * method_getnewaddress(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version)
{
	xdag_debug("rpc call method get_new_address, version %s", version);
	...
	xdag_hash_t hash;
	const int res = xdag_create_block(0, 0, 0, 0, 0, hash);
	if (res < 0) {
		ctx->error_code = 1;
		ctx->error_message = strdup("Can't generate new address.");
	}
	else {
		char address[SIZE_MAX_ADDRESS]={0};
		ndag_hash2address(hash, address);
		cJSON* item = cJSON_CreateObject();
		cJSON* json_address = cJSON_CreateString(address);
		cJSON_AddItemToObject(item, "address", json_address);
		cJSON_AddItemToArray(ret, item);
	}
...
cJSON * method_send(struct xdag_rpc_context * ctx, cJSON * params, cJSON *id, char *version)
{
	...
	char amount[128] = {0};
	char address[128] = {0};
	char fromaddress[128] = { 0 };//add new
	...
	if (params) {
		if (cJSON_IsArray(params) && cJSON_GetArraySize(params) == 1) {
			...
			//add new
			cJSON * json_fromaddress = cJSON_GetObjectItem(param, "fromaddress");
			if (cJSON_IsString(json_fromaddress)) {
				strcpy(fromaddress, json_fromaddress->valuestring);
			}
			//***
		} else {
			ctx->error_code = 1;
	...
	if(g_xdag_state < XDAG_STATE_XFER) {
		...
		} else if (strlen(fromaddress) && xdag_address2hash(fromaddress, xfer.fromHash)) {
			ctx->error_code = 1;
			ctx->error_message = strdup("Xfer: incorrect fromaddress.");
		}
		else {
...
{
	/* register query func */
	rpc_register_func(version);
	rpc_register_func(ndag_state);
	rpc_register_func(ndag_stats);
	
	/* register xdag_get_account, xdag_get_balance, xdag_do_xfer, xdag_get_transactions */
	rpc_register_func(accounts);
	rpc_register_func(getnewaddress);
	rpc_register_func(getbalance);
	rpc_register_func(send);
	rpc_register_func(listtransactions);
	return 0;
}
